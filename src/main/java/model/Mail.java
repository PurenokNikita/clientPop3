...
//    }
    private void initByLines(ArrayList<String> lines) throws Exception {
        String message = Arrays.toString(new ArrayList[]{lines});
        ...
        for (int i = 0; i < lines.size(); ++i) {
            ...
            if (buf.startsWith("Subject: ")) {
                ...
                String fromEncodingSubject = subject.substring(2, index);
                System.out.println(fromEncodingSubject);


                boolean isFromEncoding = false;
                String regex = "=\\?(?i)" + fromEncodingSubject + "\\?B\\?(.*)\\?=";
                Pattern p = Pattern.compile(regex);


                for (Matcher m = p.matcher(buf.substring(5)); m.find(); this.subject = MyBase64.getFromBASE64(m.group(1),fromEncodingSubject)) {
                    isFromEncoding = true;
                }
            ...
            if (buf.startsWith("Return-Path:")) {
                ...
                while (m.find()) {
                    ...
                    for (Matcher m = p.matcher(buf); m.find(); this.subject = MyBase64.getFromBASE64(m.group(1), encoding)) {
                        isUTF8 = true;
                    }
            ...
            if (decodeWithBase64 && !isPreLine) {
                this.content = this.content + MyBase64.getFromBASE64(buf, null);
            }
        }
...
//            }
    ...
    public String getSubject () {
        return this.subject.isEmpty() ? "без темы" : this.subject ;
    ...
    public Vector getTo_list () {
        return this.to_list;
    }

    public void setTo_list (Vector to_list){
